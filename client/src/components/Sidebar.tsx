...
const Sidebar = ({isMenuOpen, project, setProject, isGenerating, setIsGenerating}: SidebarProps) => {
  ...
  return (
    <div className={`h-full sm:max-w-sm rounded-xl bg-gray-800 transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
                    ...
                    if (isMessage) {
                        ...
                        return(
                            <div key={msg.id} className={`flex items-start gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                                ...
                                ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none" : "rounded-tl-none bg-gray-800 text-gray-100"}`}>
                                    {msg.content}
                                </div>
                                {isUser && (
